...
public class ServiceContainer {
  ...
  func initialize() {
    // Routes.
    container.register(RouteServiceAlamofire.self) {
      _ in
      RouteServiceAlamofire()
    }
    container.register(RouteService.self) {
      r in
      r.resolve(RouteServiceAlamofire.self)!
    }

    // Stations.
    container.register(StationServiceAlamofire.self) {
      _ in
      StationServiceAlamofire()
    }
    container.register(StationService.self) {
      r in
